...
function availableMoveNotInDangerZone(color) {
  ...
  for (i=0; i<boardSize; i++) {
    ...
    /*
    else if (!((row == 1 && col ==0) && gameBoard[row+1][col] == oppositeColor(turn)) || !((row == 0 && col ==1) && gameBoard[row][col+1] == oppositeColor(turn)) || !((row == 6 && col ==0) && gameBoard[row-1][col] == oppositeColor(turn)) ||! ((row == 7 && col ==1) && gameBoard[row][col+1] == oppositeColor(turn)) || !((row == 0 && col ==6) && gameBoard[row][col-1] == oppositeColor(turn)) || !((row == 1 && col ==7) && gameBoard[row+1][col] == oppositeColor(turn)) || !((row == 6 && col ==7) && gameBoard[row=1][col] == oppositeColor(turn)) || !((row == 7 && col ==6) && gameBoard[row][col-1] == oppositeColor(turn)) || !((row == 1 && col ==0) && gameBoard[row+1][col] == oppositeColor(turn)))
      if (gameBoard[i][j].color == 'empty' && checkAllLegalDirections(gameBoard, i, j, color)) 
        return true;*/
  }
}
      ...
      for (j=0; j<boardSize; j++) {
          ...
          if (board[i][j].color == 'empty') {
            ...
            //skip danger zone 
              ...
              //compute the position that gives the least tiles flipped 
              /*
              if (numberOfEasyMoves >0) {

              if (firstTime == true && countTiles >0) {
                  minimumTiles = countTiles;
                  minRowPosition = i;
                  minColPosition =j;
                  firstTime = false;
                }
                ...
                //there is a move.
                if (countTiles >0 && countTiles < minimumTiles) {
                   minimumTiles = countTiles;
                   minRowPosition = i;
                   minColPosition = j;
                }
...
if (corners.length != 0) {
  ...
  /*
else if (numberOfEasyMoves > 0 && minimumTiles>0) {
  positionReturn.row = minRowPosition;
  positionReturn.col = minColPosition;
  numberOfEasyMoves--;
...
/*
function advancedcomputerMoveColumn(board, color) {
 
  var i,j;
  
   var countTiles=0, maximumTiles=0, minimumTiles=0;
   var maxColPosition=0, minColPosition=0;
   var firstTime = true;
   ...
   //see if corner moves are possible and take the corner move with max tiles.
 ...
 if (corners.length != 0) {
  //if there are corners, take the corner with the most tiles you can flip
    ...
    for (z=0; z<corners.length;z++) {
      if (cornerTiles[z] >maxTilesCorner)  {
        maxTilesCorner = cornerTiles[z];
        maxColPosition = corners[z];
      }
    }
    return maxColPosition;
  }

  if (numberOfEasyMoves > 0 && minimumTiles>0)  {
    numberOfEasyMoves--;
    return minColPosition;
  }
...
*/
/*
function computerMoveRow(board, color) {
 
  var i,j;
  
   var countTiles=0, maximumTiles=0;
...
function computerMoveColumn(board, color) {
...
/*
function cornerPairCaptured(row,col) {
 
  if (gameBoard[0][0].color == turn && gameBoard[0][7].color == turn) {
    return true;

  }
  else if (gameBoard[7][0].color == turn && gameBoard[0][0].color == turn) {
    return true;


  }

  else if (gameBoard[0][7].color == turn && gameBoard[7][7].color == turn) {

    return true;

  }

  else if (gameBoard[7][0].color == turn && gameBoard[7][7].color == turn) {
    return true;
...
/*capture tiles near corner, because these tiles are likely to not get takebn
function playCorner() {
  var row, col;
  row =0;
  //if both top cornesr captured, try to capture as much top as possible
  //clockwise circle
  if (gameBoard[0][0].color == turn && gameBoard[0][7].color == turn) {
    //play step by stsep
    for (col =0; col<7; col++) {
      if (gameBoard[0][col].color == 'empty') {
        //play as left as possible
